...
export const FIELD_DATA = {
  ROLEAPRV: "Enum_apvRoleAprv",


  PSJBTYPE: "Enum_apvObjcType",
  PSJBCODE: "lstJob",
  PSJBCODE_DMC: "lstDcmnPrcs",

  PRCSCODE: "lstDcmnPrcs",
  PRCSLIST: "lstDcmnPrcs",

  SENDMAIL: "Enum_apvSendMail",
  DLAYTYPE: "Enum_apvTimeType",

  POSITIONS: "lstPosition",
  EMPLOYEES: "lstEmployee_Tble"
}


export default function getProcessCategories() {
  const [data, setData] = useState({
    ROLEAPRV: [], // Enum_apvRoleAprv              Loại ký duyệt

    PSJBTYPE: [],     //  Enum_apvObjcType              Chức danh/ Chức vụ/ Chỉ định
    PSJBCODE: [],    //   lstJob                        Loại ký duyệt (PSJBTYPE == 1)
    PSJBCODE_DMC: [], //  lstDcmnPrcs                   Loại ký duyệt (PSJBTYPE == 2)

    PRCSCODE: [],     // lstDcmnPrcs                   Loại ký duyệt,
    PRCSLIST: [],     // lstDcmnPrcs                   Loại ký duyệt

    SENDMAIL: [],     // Enum_apvSendMail              Đối tượng nhận thông báo
    DLAYTYPE: [],     // Enum_apvTimeType              Đơn vị thời gian
  ...
  const getCategories = () => {
    ...
    api(localStorage.getItem("usertoken"))
      ...
      .then((res) => {
        ...
        const ROLEAPRV = records.filter((rc) => {
          return rc.LISTCODE === "Enum_apvRoleAprv"
        });

        const POSITIONS = records.filter((rc) => {
          return rc.LISTCODE === FIELD_DATA.POSITIONS
        });

        const PSJBTYPE = records.filter((rc) => {
          return rc.LISTCODE === "Enum_apvObjcType"
        });

        const PSJBCODE = records.filter((rc) => {
          return rc.LISTCODE === "lstJob"
        });

        const PSJBCODE_DMC = records.filter((rc) => {
          return rc.LISTCODE === "lstDcmnPrcs"
        });

        const PRCSCODE = records.filter((rc) => {
          return rc.LISTCODE === "lstDcmnPrcs"
        });

        const PRCSLIST = records.filter((rc) => {
          return rc.LISTCODE === "lstDcmnPrcs"
        });

        const SENDMAIL = records.filter((rc) => {
          return rc.LISTCODE === "Enum_apvSendMail"
        });

        const DLAYTYPE = records.filter((rc) => {
          return rc.LISTCODE === "Enum_apvTimeType"
        });

        const EMPLOYEES = records.filter((rc) => {
          return rc.LISTCODE === "lstEmployee_Tble"
        });
  ...
  return {
    getCategories: getCategories, data: data, error: error,
  }
}
