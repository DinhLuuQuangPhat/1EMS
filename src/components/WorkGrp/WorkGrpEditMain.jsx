...
const WorkGrpEditMain = (props) => {
    ...
    const PageInfo = {
        TitlePage: getLabelValue(null, "Nhóm Công Việc"),
        UrlLink: "/work-group/",
    ...
    const DetailNCVI = useSelector((state) => state.NCVI.detailInvc); // state.NCVI, cái NCVI dựa vào NCVI trong combineReducers của reducers/index.js
    ...
    useEffect(() => {
        if (DetailNCVI) {
            setHeader(DetailNCVI !== undefined ? DetailNCVI : initHeader);
            DetailNCVI.STTESIGN <= 0 ? setPermissions(true) : setPermissions(false);
            setAcceRght(DetailNCVI.ACCERGHT);
            setStteSign(DetailNCVI.STTESIGN);
        }
    ...
    const actionSave = () => {
        if (header.WKPCNAME === "" || header.WKPCNAME === null) {
            ...
            setContentNotify({
                type: "",
                content: "Vui lòng nhập tên nhóm công việc",
            });
            return;
        }
        else {
            ...
            if (header.KKKK0000) {
                // update
                dispatch(updateNCVI(postJson));
            } else {
                // Post
                dispatch(postNCVI(postJson));
            }
        }
    };
        ...
        if (postResult) {
            ...
            if (postResult.RETNCODE) {
                if (postResult.RETNDATA !== null) {
                    dispatch(getDetailNCVI(postResult.RETNDATA[0].KKKK0000));
                    setMode("EDIT");
                } else {
                    dispatch(resetNCVI());
                                    ...
                                    <div className="w-full">
                                        <div className=" mb-3">
                                            ...
                                            <FieldEditMaskText
                                                id="WKGRCODE"
                                                name="WKGRCODE"
                                                title={getLabelValue(null, "Mã số nhóm công việc")}
                                                value={header.WKGRCODE ? header.WKGRCODE.toString() : ""}
                                            />
                                        </div>
                                    </div>
                                    <div className="w-full">
                                        <div className="mb-3">
                                            ...
                                            <FieldEditInput
                                                title={getLabelValue(null, "Tên nhóm công việc")}
                                                id={"WKGRNAME"}
                                                defaultValue={header?.WKGRNAME}
                                                value={header?.WKGRNAME}
                                                onChange={(event) =>
                                                    setHeader({
                                                        ...header,
                                                        WKGRNAME: event.value,
                                                    })
                                                }
...
export default WorkGrpEditMain;
