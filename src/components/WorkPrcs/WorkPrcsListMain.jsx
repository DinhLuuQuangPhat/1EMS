...
const WorkPrcsListMain = () => {
    ...
    const PageInfo = {
        ...
        UrlLink: "/work-prcs/",
        TitlePage: getLabelValue(141, "Tính Chất Công Việc"), // Code 
    };
    ...
    const listTCCVI = useSelector((state) => state.TCCVI.listMaster);

    const [lstChungTu, setLstChungTu] = useState([]);
    const [CountChungTu, setCountChungTu] = useState(0);

    useEffect(() => {
        if (listTCCVI && listTCCVI.length > 0) {
            setLstChungTu(listTCCVI);
            setCountChungTu(listTCCVI.length);
...
export default WorkPrcsListMain;